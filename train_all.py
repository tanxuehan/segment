...
def start_train():
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        # content = time.ctime() + ' ' + f'Epoch {epoch}, lr: {optimizer.param_groups[0]["lr"]:.7f}, loss_train: {loss_train:.5f}, loss_valid: {loss_valid:.5f}, coeff: {coeff:.6f}.'
        content = (
            time.ctime()
            + " "
        ...
        #     torch.save(model.state_dict(), f'{model_dir}_best_AUC.pth')
        #     coeff_max = coeff
        #     not_improving = 0

        if loss_train < loss_min:
            loss_min = loss_train
            torch.save(model.state_dict(), f"{model_dir}_best_loss_all.pth")

        if not_improving == early_stop:
            print("Early Stopping...")
            break

    torch.save(model.state_dict(), f"{model_dir}_final_all.pth")
